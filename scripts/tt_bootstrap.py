    ...
    def do_run(self, command, **kwargs):
        ...
        for flash_op in self.flash_data:
            ...
            with tempfile.TemporaryDirectory() as dir_name:
                ...
                if self.erase:
                    # Only erase first 64 MB of flash
                    # We want to avoid erasing the STM32 internal flash
                    self.logger.info("Erasing SPI flash")
                    FlashEraser(session, FlashEraser.Mode.SECTOR).erase(["0x0+0x4000000"])
